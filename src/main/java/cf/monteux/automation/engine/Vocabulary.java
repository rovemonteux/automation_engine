...
public class Vocabulary {
    ...
    /**
     * Populates the vocabulary memory, containing the relationship between the
     * terms and strategies.
     *
     * @param	inputStream	Input stream to read data from
     * @throws DOMException	Error with the DOM XML parser
     * @throws ParserConfigurationException	Error with the XML parser
     * configuration
     * @throws SAXException	Error with the SAX XML parser
     * @throws IOException	Error on reading the vocabulary configuration from
     * XML
    ...
    public List<ArrayList<String>> search(String arguments, String language) {
        ...
        if (items.size() > 0) {
            ...
            if (finalResult != null && finalResult.length() > 0) {
                ...
                for (int i = 0; i < this.getVocabulary().get(finalResult).size(); i++) {
                    result.add(this.getVocabulary().get(finalResult).get(i));
                    mode.add(this.getTaskMode().get(finalResult).get(i));
                }
            }
